...
void __fastcall hkPaintTraverse(void* pPanels, int edx, unsigned int vguiPanel, bool forceRepaint, bool allowForce)
{
	...
	else if (FocusOverlayPanel == vguiPanel)
	{
		...
		if (gCvars.watermark)
		{
			Draw::DrawStringA(Fonts::m_MenuFont,false,20, 20, 255, 255, 255, 255,"INTERWEBZ");
		}
		...
		if (pLocal)
		{
			if (gCvars.ESPBOX || gCvars.ESPHEALTH)
			{
				g_ESP.draw(pLocal);
			}
			...
			if (gCvars.radar)
			{
				g_ESP.DrawRadarBack(iScreenWidth, iScreenHeight);
				g_ESP.drawradar(iScreenWidth, iScreenHeight, pLocal);
			}
			...
			if (pLocal->IsAlive())
			{
				if (gCvars.velocity)
				{
					g_ESP.drawvelocity(iScreenWidth, iScreenHeight, pLocal);
				}
			}
		}
